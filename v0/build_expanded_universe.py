...
Output: datasets/expanded/ with all parquet files + artifacts.
"""
import numpy as np
import pandas as pd
...
os.makedirs('datasets/expanded', exist_ok=True)
...
spreads_df.to_parquet('datasets/expanded/spreads.parquet')
...
for idx, name in enumerate(pair_names):
    ...
    safe = name.replace('/', '_')
    ou_df.to_parquet(f'datasets/expanded/ou_params_{safe}.parquet')
...
for idx, name in enumerate(pair_names):
    ...
    rm = s_aligned.rolling(60).mean(); rs = s_aligned.rolling(60).std()
    feat['z_score'] = (s_aligned - rm) / rs.replace(0, np.nan)
    feat['half_life'] = ou_df.reindex(common)['half_life']
    feat['ou_resid_var'] = rs ** 2
    ...
    safe = name.replace('/', '_')
    feat.to_parquet(f'datasets/expanded/features_{safe}.parquet')
...
with open('datasets/expanded/artifacts.pkl', 'wb') as f:
    pickle.dump(artifacts, f)

print(f"\n{'='*60}")
print(f"DONE: {len(pair_names)} pairs")
print(f"Artifacts saved to datasets/expanded/")
