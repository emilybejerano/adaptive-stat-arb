...
CHECKPOINT_PATH = f"checkpoints/adaptive_threshold_dqn_{mode_suffix()}.pt"
RESULTS_PATH = f"datasets/reward_results_{mode_suffix()}.pkl"
...
# ============================================================
# STATE (5 elements, following paper Section 4.2)
# ============================================================
# Paper state: (H, S, L, CC, T)
# Our state:
#   W  - Week of the month (like H - hour of day)
#   S  - Cumulative PnL from winning trades this month (normalized)
#   L  - Cumulative losses from losing trades this month (normalized)
#   CC - Number of trades taken / max trades (like consumed capacity)
#   T  - Current threshold index (normalized to [0,1])
#
# Plus market context (extending paper slightly):
#   VIX     - Current VIX level (normalized)
#   THETA   - Mean theta across pairs (normalized)
STATE_DIM = 7
...
def simulate_week(spread, z_series, vix_series, ou_df,
    ...
    return wins_pnl, losses_pnl, n_trades, losing_trades, port, (pos, ep, ps, dh)


def get_market_context(vix_series, ou_df, date_idx, dates):
    """Get normalized VIX and mean theta for state."""
    date = dates[min(date_idx, len(dates)-1)]
    vix = 0.0
    if date in vix_series.index:
        vix = vix_series.loc[date]
    vix_norm = min(vix / 80.0, 1.0)  # normalize to [0,1], cap at 80

    theta = 0.0
    if date in ou_df.index:
        theta = ou_df.loc[date, 'theta']
    theta_norm = min(theta / 20.0, 1.0)  # normalize, cap at 20
...
def run_episode(agent, pair_name, month_start, month_end, training=True):
    ...
    prev_action = None
    reward_tracker = RewardTracker(window=ROLLING_WINDOW)
    ...
    for week in range(WEEKS_PER_EPISODE):
        ...
        mid_idx = (w_start + w_end) // 2
        vix_norm, theta_norm = get_market_context(vix, ou_df, mid_idx, dates)

        state = np.array([w_norm, s_norm, l_norm, cc_norm, t_norm,
                          vix_norm, theta_norm], dtype=np.float32)
        ...
        week_wins, week_losses, week_trades, week_losing_trades, portfolio, pos_state = simulate_week(
            s, z, vix, ou_df, w_start, w_end, threshold, portfolio, pos_state)

        cum_wins += week_wins
        cum_losses += week_losses
        n_trades_total += week_trades
        episode_trade_count += week_trades
        episode_traps += week_losing_trades

        week_return = (week_wins - week_losses) / CAPITAL
        episode_returns.append(week_return)
        reward_tracker.update(week_return)

        capacity_excess = max(n_trades_total - max_trades_per_month, 0)
        original_reward = compute_original_reward(
            week_wins, week_losses, CAPITAL, week, capacity_excess
        )
        trap_indicator = 1.0 if week_losing_trades > 0 else 0.0
        reward, reward_meta = apply_reward_mode(
            original_reward, action, prev_action, trap_indicator, reward_tracker
        )
        ...
        next_vix, next_theta = get_market_context(
            vix, ou_df, min(w_end, n_days-1), dates)

        next_state = np.array([next_w_norm, next_s_norm, next_l_norm,
                               next_cc_norm, t_norm_next,
                               next_vix, next_theta], dtype=np.float32)
        ...
        transitions.append({
            ...
            'losing_trades': week_losing_trades,
            'trap_indicator': trap_indicator,
            'week_return': week_return,
            'original_reward': original_reward,
            'reward': reward,
            'switches_so_far': episode_switches,
            **reward_meta,
        })
        prev_action = action

    trap_rate = (episode_traps / episode_trade_count) if episode_trade_count > 0 else 0.0
    returns_arr = np.asarray(episode_returns, dtype=float)
    episode_sharpe = (
        float(np.mean(returns_arr) / (np.std(returns_arr) + 1e-8))
        if len(returns_arr) >= 2 else 0.0
    )
    diagnostics = {
        'episode_reward': float(episode_reward),
        'switches': int(episode_switches),
        'trap_rate': float(trap_rate),
        'trades': int(episode_trade_count),
        'avg_reward': float(np.mean([t['reward'] for t in transitions])) if transitions else 0.0,
        'sharpe_proxy': episode_sharpe,
    }
    return episode_reward, transitions, diagnostics
...
def backtest_pair(pair_name, t0, t1, threshold=None, agent=None):
    ...
    if agent is not None:
        ...
        for week_start in range(0, n_days, week_size):
            ...
            vix_n, theta_n = get_market_context(vix, ou_df, mid, dates)
            state = np.array([w_norm, s_norm, l_norm, cc_norm, t_norm,
...
print(f"  State: [week, cum_wins, cum_losses, capacity_used, threshold, VIX, theta]")
print(f"  Reward mode: {REWARD_MODE}")
print(f"  Reward formula: {reward_formula_text()}")
print(f"  lambda_switch={LAMBDA_SWITCH} lambda_trap={LAMBDA_TRAP} lambda_vol={LAMBDA_VOL} rolling_window={ROLLING_WINDOW}")
...
for iteration in range(N_ITERATIONS):
    ...
    for month_start, month_end in sampled_months:
        for pair in sampled_pairs:
            ep_reward, _, diag = run_episode(agent, pair, month_start, month_end, training=True)
            iter_reward += ep_reward
            n_episodes += 1
            iter_switches += diag['switches']
            iter_trap_rates.append(diag['trap_rate'])
            iter_sharpes.append(diag['sharpe_proxy'])
            iter_avg_rewards.append(diag['avg_reward'])
    ...
    if (iteration + 1) % 10 == 0:
        ...
        for month_start, month_end in val_months:
            for pair in cointegrated_pairs[:5]:
                ep_r, _, diag = run_episode(agent, pair, month_start, month_end, training=False)
                val_reward += ep_r
                val_switches += diag['switches']
                val_trap_rates.append(diag['trap_rate'])
                val_sharpes.append(diag['sharpe_proxy'])
                val_avg_rewards.append(diag['avg_reward'])
        ...
        avg_reward_stat = np.mean(iter_avg_rewards) if iter_avg_rewards else 0.0
        avg_trap_stat = np.mean(iter_trap_rates) if iter_trap_rates else 0.0
        avg_sharpe_stat = np.mean(iter_sharpes) if iter_sharpes else 0.0
        val_avg_reward_stat = np.mean(val_avg_rewards) if val_avg_rewards else 0.0
        val_trap_stat = np.mean(val_trap_rates) if val_trap_rates else 0.0
        val_sharpe_stat = np.mean(val_sharpes) if val_sharpes else 0.0
        ...
        print(f"             avg_reward={avg_reward_stat:.4f} switches={iter_switches} trap_rate={avg_trap_stat:.1%} sharpe={avg_sharpe_stat:.4f}")
        print(f"             val_avg_reward={val_avg_reward_stat:.4f} val_switches={val_switches} val_trap_rate={val_trap_stat:.1%} val_sharpe={val_sharpe_stat:.4f}")
...
torch.save(payload, 'adaptive_threshold_dqn.pt')

with open(RESULTS_PATH, 'wb') as f:
    pickle.dump({
        'reward_mode': REWARD_MODE,
        'wins_vs_static': wins if n > 0 else None,
        'n_pairs': n if n > 0 else 0,
        'mean_sharpe_static_1.0': float(np.mean(ss[1.0])) if n > 0 else None,
        'mean_sharpe_static_1.5': float(np.mean(ss[1.5])) if n > 0 else None,
        'mean_sharpe_dqn': float(np.mean(ds)) if n > 0 else None,
        'mean_trap_static': float(np.mean(st_trap)) if n > 0 else None,
        'mean_trap_dqn': float(np.mean(dt_trap)) if n > 0 else None,
        'wilcoxon_sharpe_p': float(pv_s) if n >= 5 else None,
        'wilcoxon_trap_p': float(pv_t) if n >= 5 else None,
        'avg_threshold_switches': float(np.mean(np.abs(np.diff(all_thresholds)))) if len(all_thresholds) > 1 else 0.0,
    }, f)
