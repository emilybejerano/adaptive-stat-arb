"""
Stage 2 Walk-Forward v2: DQN with Sit-Out Action.

Same walk-forward windows as v1, but the agent can now choose to
sit out months where it expects negative returns. This should
improve Sharpe by cutting losses rather than just picking the right γ.

Actions: {sit_out, γ=0.5, γ=1.0, γ=1.25, γ=2.0}
"""
import os
import sys
import numpy as np
...
from stage2_dqn_env_v2 import ThresholdEnvV2, GAMMA_CHOICES_V2
from stage2_dqn_env import ThresholdEnv, GAMMA_CHOICES  # v1 for static baselines


def evaluate_agent_v2(agent, env, deterministic=True):
    obs, _ = env.reset()
...
def evaluate_static(env, gamma):
    """Run a fixed γ through v1 env for comparison."""
...
def train_dqn_v2(cluster_dir, log_returns_df, train_start, train_end):
    """Train a DQN with sit-out action."""
    env = ThresholdEnvV2(cluster_dir, log_returns_df,
...
def main():
    ...
    print(f"DQN v2: 5 actions (sit_out + 4 gamma levels)\n")
    ...
    for train_start, train_end, test_start, test_end, label in windows:
        ...
        # Train DQN v2 (with sit-out)
        print(f"  Training DQN v2...", end=' ', flush=True)
        model, train_env = train_dqn_v2(cluster_dir, log_returns_df, train_start, train_end)
        ...
        # Test env v2 (for DQN)
        test_env_v2 = ThresholdEnvV2(cluster_dir, log_returns_df,
                                      start_month=test_start, end_month=test_end,
                                      reward_lambda=0.5)
        # Test env v1 (for static baselines)
        test_env_v1 = ThresholdEnv(cluster_dir, log_returns_df,
                                    start_month=test_start, end_month=test_end,
                                    reward_lambda=0.5)

        print(f"  Test: {len(test_env_v2.trading_months)} months")

        # Evaluate DQN v2
        dqn_df = evaluate_agent_v2(model, test_env_v2, deterministic=True)
        ...
        sit_out_count = (dqn_df['gamma'] == 'sit_out').sum()
        neg_months_avoided = ((dqn_df['gamma'] == 'sit_out') &
                              (dqn_df['return'] == 0.0)).sum()

        print(f"  DQN v2:      Sharpe {dqn_metrics['Sharpe']:>6.2f} | "
              f"AR {dqn_metrics['AR']:>.3f} | MDD {dqn_metrics['MDD']:>.3f} | "
              f"sat out {sit_out_count}/{len(dqn_df)} months")
        print(f"    actions: {dict(gamma_counts)}")

        row = {'window': label, 'test': f'{test_start}-{test_end}',
               'DQN_v2_Sharpe': dqn_metrics['Sharpe'], 'DQN_v2_AR': dqn_metrics['AR'],
               'DQN_v2_MDD': dqn_metrics['MDD'], 'sit_out_count': sit_out_count}

        # Evaluate static baselines (same as v1)
        for gamma in GAMMA_CHOICES:
            ...
            print(f"  static {gamma:<4}: Sharpe {static_metrics['Sharpe']:>6.2f} | "
    ...
    # --- Aggregate ---
    print(f"{'='*60}")
    print(f"  AGGREGATE — All test windows ({method_name})")
    print(f"{'='*60}\n")

    agg_dqn = compute_metrics(all_dqn_test_returns)
    print(f"  {'Method':<25} {'Sharpe':>7} {'AR':>7} {'MDD':>7}")
    print(f"  {'-'*50}")
    print(f"  {'DQN v2 (sit-out)':<25} {agg_dqn['Sharpe']:>7.2f} {agg_dqn['AR']:>7.3f} {agg_dqn['MDD']:>7.3f}")
    ...
    # Per-window table
    print(f"\n  Per-window Sharpe:")
    print(f"  {'Window':<20} {'DQN v2':>7} {'Sat out':>7}", end='')
    ...
    for row in window_results:
        ...
        wins = row['DQN_v2_Sharpe'] > best_static
        if wins:
            dqn_wins += 1
        print(f"  {row['window']:<20} {row['DQN_v2_Sharpe']:>7.2f} {row['sit_out_count']:>7}", end='')
        for g in GAMMA_CHOICES:
            print(f" {row.get(f'Static_{g}_Sharpe', 0):>8.2f}", end='')
        print(f" {'YES' if wins else 'no':>7}")

    print(f"\n  DQN v2 beats best static in {dqn_wins}/{len(window_results)} windows")

    # --- Save ---
    os.makedirs('./res/stage2', exist_ok=True)
    results_df = pd.DataFrame(window_results)
    results_df.to_csv(f'./res/stage2/walkforward_v2_{method_name}.csv', index=False)
...
def _plot_walkforward(window_results, dqn_returns, static_returns, method_name):
    ...
    # Panel 1: Per-window Sharpe
    ax = axes[0]
    windows = [r['window'] for r in window_results]
    x = np.arange(len(windows))
    width = 0.15

    dqn_sharpes = [r['DQN_v2_Sharpe'] for r in window_results]
    ax.bar(x - 2*width, dqn_sharpes, width, label='DQN v2 (sit-out)', color='steelblue', zorder=3)

    colors = ['#ff7f0e', '#2ca02c', '#d62728', '#9467bd']
    for i, gamma in enumerate(GAMMA_CHOICES):
        sharpes = [r.get(f'Static_{gamma}_Sharpe', 0) for r in window_results]
        ax.bar(x + (i-1)*width, sharpes, width, label=f'Static g={gamma}',
               color=colors[i], alpha=0.7, zorder=3)

    ax.set_xticks(x)
    ax.set_xticklabels(windows, rotation=15, ha='right')
    ax.set_ylabel('Sharpe Ratio')
    ax.set_title('Per-Window Sharpe: DQN v2 (sit-out) vs Static')
    ax.legend(fontsize=7, loc='upper left')
    ax.grid(True, alpha=0.3, axis='y')

    # Panel 2: Cumulative wealth
    ax = axes[1]
    dqn_wealth = np.exp(np.cumsum(dqn_returns))
    ax.plot(dqn_wealth, label='DQN v2 (sit-out)', linewidth=2, color='steelblue')

    for i, gamma in enumerate(GAMMA_CHOICES):
        wealth = np.exp(np.cumsum(static_returns[gamma]))
        ax.plot(wealth, label=f'Static g={gamma}', color=colors[i], alpha=0.7, linewidth=1)
    ...
    plt.suptitle(f'Walk-Forward v2 (Sit-Out) — {method_name}', fontsize=13, fontweight='bold')
    plt.tight_layout()
    plt.savefig(f'./res/stage2/walkforward_v2_{method_name}.png', dpi=150, bbox_inches='tight')
    print(f"Saved ./res/stage2/walkforward_v2_{method_name}.png")
